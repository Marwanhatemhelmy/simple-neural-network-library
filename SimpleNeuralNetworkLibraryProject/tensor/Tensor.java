...
import utils.tensors.squeezing.TensorSqueezing;
...
public class Tensor{
    ...
    public void zeroGrad_(){
        for (int i=0;i<this.indeces.length;i++){
            this.rootTensor.grad[this.indeces[i]] = 0;
        }
        this.refreshTensor();
    }
    ...
    public void fill_(double val){
        for (int i=0;i<this.indeces.length;i++){
            if (!this.gradFlag){
                this.rootTensor.data[this.indeces[i]] = val;
            }else{
                this.rootTensor.grad[this.indeces[i]] = val;
            }
        }
        this.refreshTensor();
    }
    ...
    public void squeez_(int dim){
        ...
        this.shape = TensorSqueezing.squeezeOnDimension(this.shape, dim);
        this.dimensions = this.shape.length;
    }
    ...
    public void squeez_(){
        ...
        this.shape = TensorSqueezing.squeeze(this.shape);
